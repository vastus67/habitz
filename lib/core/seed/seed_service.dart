...
class SeedService {
  ...
  Future<void> _seedPlansAndWorkouts() async {
    final plans = [
      _SeedPlan(
        'reset_bodyweight_foundation',
        'Reset Bodyweight Foundation',
        'fatloss',
        'beginner',
        'unisex',
        3,
        4,
        'none',
        'A no-equipment plan built to restart consistency, daily movement, and fat-loss momentum.',
        ['bodyweight', 'reset', 'fatloss'],
      ),
      _SeedPlan(
        'male_gym_strength_foundation',
        'Male Gym Strength Foundation',
        'strength',
        'beginner',
        'male',
        4,
        6,
        'gym',
        'A male-focused upper/lower split for gym users who want visible strength progress and clean weekly structure.',
        ['male', 'gym', 'strength'],
      ),
      _SeedPlan(
        'male_home_recomp_system',
        'Male Home Recomp System',
        'fatloss',
        'intermediate',
        'male',
        4,
        6,
        'home',
        'A home training plan for men focused on conditioning, muscle retention, and stronger weekly adherence.',
        ['male', 'home', 'recomp'],
      ),
      _SeedPlan(
        'female_strength_sculpt',
        'Female Strength Sculpt',
        'strength',
        'beginner',
        'female',
        4,
        6,
        'gym',
        'A female-focused gym plan built around lower-body emphasis, strength, and sustainable recovery.',
        ['female', 'gym', 'strength'],
      ),
      _SeedPlan(
        'female_home_balance_burn',
        'Female Home Balance Burn',
        'fatloss',
        'beginner',
        'female',
        3,
        6,
        'home',
        'A home-friendly female plan for fat loss, confidence, and balanced lower-body progression.',
        ['female', 'home', 'fatloss'],
      ),
      _SeedPlan(
        'unisex_mobility_reset',
        'Unisex Mobility Reset',
        'mobility',
        'beginner',
        'unisex',
        3,
        4,
        'home',
        'A mobility-first program to improve recovery, posture, and daily movement quality.',
        ['mobility', 'reset', 'home'],
      ),
    ];
  ...
  List<String> _dayTitlesForPlan(String planName, int daysPerWeek) {
    if (planName == 'Male Gym Strength Foundation' ||
        planName == 'Female Strength Sculpt') {
      return ['Upper Body', 'Lower Body', 'Pull Focus', 'Push + Core'];
    }
    if (planName == 'Male Home Recomp System' ||
        planName == 'Female Home Balance Burn') {
      return ['Metabolic Circuit', 'Lower + Core', 'Cardio Conditioning'];
    }
    if (planName == 'Unisex Mobility Reset') {
      return ['Shoulder + Spine', 'Hips + Ankles', 'Core + Reset'];
    }
  ...
  List<_SeedExercise> _sampleExercisesForDay(String planName, int dayIndex) {
    ...
    if (planName == 'Unisex Mobility Reset') {
      final mobilityPhase = (dayIndex - 1) % 3;
      switch (mobilityPhase) {
        case 0:
          return [
            _SeedExercise('Band Pull Apart', 'upper-back,shoulders', 'home', 'Drive movement from the shoulder blades.', 3, '15', 30),
            _SeedExercise('Thoracic Rotation', 'spine,core', 'none', 'Rotate slowly without losing rib control.', 3, '8/side', 30),
            _SeedExercise('Wall Slide', 'shoulders', 'none', 'Keep lower back gently braced.', 3, '10', 30),
          ];
        case 1:
          return [
            _SeedExercise('Goblet Squat Hold', 'legs,hips', 'home', 'Breathe calmly into the bottom position.', 3, '30s', 30),
            _SeedExercise('90/90 Hip Switch', 'hips', 'none', 'Move from the hips rather than the spine.', 3, '10', 30),
            _SeedExercise('Calf Rocker', 'calves,ankles', 'none', 'Drive the knee forward while heel stays grounded.', 3, '12/side', 20),
          ];
        default:
          return [
            _SeedExercise('Bird Dog', 'core,glutes', 'none', 'Reach long and keep the trunk stable.', 3, '10/side', 20),
            _SeedExercise('Pallof Press', 'core', 'home', 'Resist rotation all the way out.', 3, '12/side', 30),
            _SeedExercise('Child Pose Breathing', 'recovery', 'none', 'Use long exhale and relaxed shoulders.', 3, '5 breaths', 20),
          ];
      }
    }

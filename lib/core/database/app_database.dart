...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> clearUserProgress() async {
    await transaction(() async {
      await delete(habitLogEntries).go();
      await delete(habitEntries).go();
      await delete(workoutSessionLogEntries).go();
  ...
  MigrationStrategy get migration => MigrationStrategy(
        ...
        onUpgrade: (m, from, to) async {
          ...
          if (from < 3) {
            await m.deleteTable('workout_session_log_entries');
            await m.deleteTable('workout_set_entries');
            await m.deleteTable('workout_day_entries');
            await m.deleteTable('exercise_entries');
            await m.deleteTable('workout_plan_entries');
            await m.deleteTable('user_profile_entries');
            await m.createTable(workoutPlanEntries);
            await m.createTable(workoutDayEntries);
            await m.createTable(exerciseEntries);
            await m.createTable(workoutSetEntries);
            await m.createTable(workoutSessionLogEntries);
            await m.createTable(userProfileEntries);
          }
        },
      );

...
class _OnboardingScreenState extends ConsumerState<OnboardingScreen> {
  ...
  Widget _accountStep(OnboardingState state, OnboardingController controller) {
    return OnboardingStepCard(
      ...
      subtitle: 'Create an account now or continue as guest.',
      visual: _heroVisual(Icons.account_circle_rounded),
      child: Column(
        children: [
          OnboardingOptionCard(
            title: 'Continue with Google',
            selected: state.accountChoice == AccountChoice.google,
            onTap: () => controller.setAccountChoice(AccountChoice.google),
          ),
          const SizedBox(height: 10),
          OnboardingOptionCard(
            title: 'Continue with Apple',
            selected: state.accountChoice == AccountChoice.apple,
            onTap: () => controller.setAccountChoice(AccountChoice.apple),
          ),
          const SizedBox(height: 10),
          OnboardingOptionCard(
            title: 'Continue with Email',
            selected: state.accountChoice == AccountChoice.email,
            onTap: () => controller.setAccountChoice(AccountChoice.email),
          ),
          const SizedBox(height: 10),
          TextButton(
            onPressed: () => controller.setAccountChoice(AccountChoice.guest),
            child: const Text('Skip for guest mode'),
          ),
        ],
      ),
      ...
      canContinue: state.accountChoice != null,
      onBack: () => _goToStep(8),
    );
  }
